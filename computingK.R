computingK <- function(){
  ...
  for (i in 1:nrow(output_data)) {
    ...
    if ( floor(sum) != 100) {
      cat("---- WARNING! Data with values: \n", names(input_data[1]),":", sand, "\n", names(input_data[2]), ": ", silt ,"\n", names(input_data[3]),":", clay, "\n", "has wrong quantities(sand + silt + clay must be equal to 100), it will be left blank\n")
      
    } else if(decimalplaces(sand) || decimalplaces(silt) || decimalplaces(clay) ) {
      cat("---- WARNING! All SandUSDA, SiltUSDA and ClayUSDA must have at most 1 decimal. That data will be left with a blank space\n")
      ...
      if (0 <= a && a <= 4){
        ...
        if (is.na(VFS)) {
          ...
          if(is.na(vectorq1[i]) || is.na(vectorq2[i]) || is.na(vectorq3[i])) {
            ...
            cat("---- WARNING! Result of parameter M is bigger tan 8000. You can call help_K() for generic information about the usage of this program. That data will be left with a blank space\n")
            cat("The data of that wrong entry is: \n")
            cat ( names(input_data[1]),":", sand, "\n", names(input_data[2]), ": ", silt ,"\n", names(input_data[3]),":", clay, "\n", names(input_data[4]),":", VFS ,"\n", names(input_data[5]),": ", a, "\n", names(input_data[6]), ": ", soil, "\n", names(input_data[7]), ": ", permeability,"\n")
          }
        } else {
        ...
        cat("For more information call help_K(\"input data\")\n")
